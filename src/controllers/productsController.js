...
export const getProducts = async (req, res) => {
    try {
        ...
        if (products.docs.length === 0) {
            return res.status(404).render("templates/error", {error: "No se encontraron productos que cumplan con el filtro de busqueda"})
        }
...
export const addProduct = async (req, res) => {
    try {
        ...
        if (!title || !description || !code || !price || !stock || !category) {
            return res.status(400).render("templates/error", {error: "Algun dato no se ha completado"})
        }
